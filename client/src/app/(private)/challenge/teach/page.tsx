...
export default function TeachAiPage() {
  ...
  // Challenge Stages
  const [twoHandUnlocked, setTwoHandUnlocked] = useState(false);
  const [isTrainedLevel2, setIsTrainedLevel2] = useState(false);
  const [submitLevel, setSubmitLevel] = useState<1 | 2>(1);
  ...
  // Celebrate when 2-hand classes get unlocked
  useEffect(() => {
    if (twoHandUnlocked && !prevUnlockedRef.current) {
      prevUnlockedRef.current = true;
      setShowUnlockCelebration(true);
      playSuccessSound();
      speakVietnamese('Tuyệt vời! Bé đã mở khóa phần 2 bàn tay rồi! Hãy thử dạy A I nhận biết cử chỉ 2 bàn tay nhé!');
  ...
  const captureSample = () => {
    ...
    const isTwoHandClass = activeClass === 'class_3' || activeClass === 'class_4';
    if (isTwoHandClass && hands.length < 2) return;
    ...
    setSamples(prev => {
      ...
      processHand(0);
      if (isTwoHandClass) processHand(1);
  ...
  const handleTrain = () => {
    ...
    if (!twoHandUnlocked) {
      if (c1 < 10 || c2 < 10) {
        speakVietnamese('Bé chưa chụp đủ 10 ảnh mẫu cho mỗi nhóm 1 bàn tay rồi! Bé hãy chụp thêm hình mẫu nhé!');
        return;
      }
    } else {
      if (c3 < 6 || c4 < 6) { // c3/c4 need 6 samples (3 captures x 2 hands)
        speakVietnamese('Bé chưa chụp đủ 6 ảnh mẫu cho mỗi nhóm 2 bàn tay rồi! Bé hãy chụp thêm hình mẫu nhé!');
        return;
      }
    }
    ...
    const interval = setInterval(() => {
      setTrainingProgress(prev => {
        if (prev >= 100) {
          ...
          setIsTrained(true);
          if (twoHandUnlocked) {
            setIsTrainedLevel2(true);
          }
          playSuccessSound();
          speakVietnamese('A I đã học xong rồi! Bé hãy kiểm tra kết quả nhé!');
          return 100;
        }
        return prev + 10;
      });
  ...
  const handleOpenSubmit = (level: 1 | 2) => {
    playClickSound();
    setSubmitLevel(level);
    
    // Evaluate accuracy against Golden Dataset
    const targetClasses = level === 1 
      ? [CLASSES[0].label, CLASSES[1].label] 
      : [CLASSES[2].label, CLASSES[3].label];
    ...
    sampleCounts['class_3'] = Math.floor(sampleCounts['class_3'] / 2);
    sampleCounts['class_4'] = Math.floor(sampleCounts['class_4'] / 2);

    let hasPenalty = false;
    const classesToCheck = level === 1 ? ['class_1', 'class_2'] : ['class_3', 'class_4'];
          ...
          <div className="bg-white rounded-3xl p-6 border-4 border-indigo-400 shadow-xl flex flex-col justify-between">
            <div>
                ...
                {CLASSES.slice(2, 4).map(cls => {
                  ...
                  return (
                    <div
                      key={cls.id}
                      onClick={() => {
                        if (!twoHandUnlocked) return;
                        playClickSound();
                        setActiveClass(cls.id);
                        speakVietnamese(cls.voicePrompt);
                      }}
                      className={`cursor-pointer rounded-2xl p-4 border-2 transition-all flex items-center justify-between ${
                        isSelected && twoHandUnlocked
                          ? 'border-indigo-500 bg-indigo-50/80 shadow-md ring-2 ring-indigo-200'
                          : 'border-gray-200 bg-gray-50 hover:bg-gray-100'
                      }`}
                    >
              ...
              {showUnlockCelebration && (
                <div className="mb-4 p-4 bg-gradient-to-r from-yellow-100 to-amber-100 border-2 border-yellow-400 rounded-2xl text-center animate-bounce">
                  <span className="text-2xl">🎉🔓🌟</span>
                  <p className="text-sm font-black text-yellow-800 mt-1">Mở khóa thành công: 2 Bàn tay!</p>
                  <p className="text-xs font-semibold text-yellow-700">Bé giỏi quá! Giờ hãy thử dạy AI với 2 bàn tay nhé!</p>
                </div>
              )}
              ...
              {(() => {
                const getCount = (id: string, label: string) => {
                  ...
                  return (id === 'class_3' || id === 'class_4') ? Math.floor(raw / 2) : raw;
                };
                ...
                const isReady = c1 >= 10 && c2 >= 10 && c3 >= 10 && c4 >= 10;
                  ...
                  return (
                    <div className="bg-red-50 border-2 border-red-200 text-red-700 rounded-2xl p-4 text-xs font-bold mb-6 flex flex-col gap-1.5 shadow-inner">
                      ...
                      <ul className="list-disc pl-4 space-y-1">
                        ...
                        {c3 < 10 && <li>Nhóm "{CLASSES[2].label}": thiếu {10 - c3} ảnh mẫu.</li>}
                        {c4 < 10 && <li>Nhóm "{CLASSES[3].label}": thiếu {10 - c4} ảnh mẫu.</li>}
                      </ul>
                    </div>
                  );
                }
            ...
            <div>
              ...
              {isTrained && !isTraining && !isTrainedLevel2 && (
                <button
                  onClick={() => handleOpenSubmit(1)}
              ...
              {isTrainedLevel2 && !isTraining && (
                <button
                  onClick={() => handleOpenSubmit(2)}
                  className="w-full bg-yellow-400 hover:bg-yellow-500 text-yellow-900 font-extrabold py-3 px-6 rounded-2xl shadow-md mt-3 border-b-4 border-yellow-600 flex items-center justify-center gap-2"
                >
                  <Award className="w-5 h-5" />
            ...
            {submitSuccess ? (
              <div className="text-center py-8">
                ...
                <h3 className="text-2xl font-black text-indigo-900 mt-4">
                  {submitLevel === 1 ? 'Nộp Bài Bước 1 Hoàn Tất!' : 'Nộp Bài Bước 2 Hoàn Tất!'}
                </h3>
