...
const CLASSES = [
  { id: 'class_1', label: 'Thích (Thumbs Up)', emoji: '👍' },
  { id: 'class_2', label: 'Quyết Tâm (Fist)', emoji: '✊' },
  { id: 'class_3', label: 'Chiến Thắng (Peace)', emoji: '✌️' },
  { id: 'class_4', label: 'Chào Bạn (Open Hand)', emoji: '✋' },
  { id: 'class_5', label: 'Rock & Roll', emoji: '🤘' },
  { id: 'class_6', label: 'Chỉ Tay (Point)', emoji: '☝️' },
];

export default function TeachGesturesPage() {
  ...
  const handleSubmitAssignment = async () => {
    ...
    try {
      ...
      const created = await api.createDataset('teach-gestures', processedSamples, submitScore, `${reflectionAnswer} | Lời nhắn: ${teacherMessage}`);
      if (created?.model?.id) {
  ...
  const handleFinalize = async () => {
    try {
      ...
      const chain = await api.getModelChain('teach-gestures').catch(() => []);
      ...
      <div className="max-w-[1600px] w-[98%] mx-auto">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between mb-8 gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl md:text-3xl font-black text-indigo-900">Dạy AI Nhận Biết Cử Chỉ</h1>
          </div>
        </div>
              ...
              {submitSuccess ? (
                <div className="text-center py-8">
                  ...
                  <p className="text-gray-600 font-semibold mt-2">
                    Bé đã hoàn thành tốt phần Dạy Cử Chỉ! Phần thưởng: Mở khóa thử thách Khuôn Mặt 🎭
                  </p>
                  <button
                    onClick={() => {
                      router.push('/challenge/gestures');
                    }}
                    className="mt-6 px-6 py-2.5 bg-pink-600 text-white font-extrabold rounded-full hover:scale-105 transition-transform"
                  >
                    Chơi Game Ảo Thuật Ngay 🚀
                  </button>
                </div>
              ) : (
                <div>
                  ...
                  <div className="mb-4">
                    <label className="text-xs font-black text-gray-700 block mb-1.5 flex items-center gap-1">
                      ...
                      <span>Bé hãy trả lời: Làm sao để phân biệt các cử chỉ rõ ràng nhất?</span>
                    </label>
                    <select
                    ...
                    >
                      <option value="Chụp ảnh rõ nét và giữ tay thật yên lặng khi chụp">
                        Chụp hình đủ sáng, xoay tay nhiều góc độ và mở rộng ngón tay rõ ràng ☝️
                      </option>
                      <option value="Chạy nhảy rung lắc camera thật mạnh">
                        Làm thật nhanh và để tay mờ mịt 🤪
                      </option>
                      <option value="Chụp nhiều vật thể lộn xộn trong phòng">
                        Chụp các ngón tay dính vào nhau khó nhìn 🧸
                      </option>
                    </select>
                  </div>
