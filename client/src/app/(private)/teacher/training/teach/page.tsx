...
export default function TeacherTeachPage() {
  ...
  const captureSample = () => {
    ...
    if (!isStable) {
      ...
      setValidationToast(`⚠️ Tay đang rung. Hãy giữ yên tay rồi chụp lại!`);
      ...
      <div className="max-w-[1600px] w-[98%] mx-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-2xl md:text-3xl font-black text-indigo-900">Huấn luyện AI cho Giáo viên</h1>
          </div>
        </div>
            ...
            <div>
              ...
              {activeDataTab === 'camera' && (
                <button
                  ...
                  className={`
                    w-full py-4 rounded-2xl font-black text-lg transition-all flex items-center justify-center gap-2 mb-4
                    ${isCapturing 
                      ? 'bg-red-500 text-white shadow-inner scale-95 ring-4 ring-red-200' 
                      : 'bg-indigo-600 text-white hover:bg-indigo-500 shadow-xl shadow-indigo-200 hover:-translate-y-1'
                    }
                    ${modelStatus !== 'ready' ? 'opacity-50 cursor-not-allowed bg-gray-400 hover:bg-gray-400 hover:-translate-y-0 shadow-none' : ''}
                  `}
                >
              ...
              <p className="text-xs font-semibold text-gray-500 text-center leading-relaxed mb-6 bg-yellow-50 border border-yellow-200 rounded-xl p-3">
                💡 <b>Mẹo cho bạn:</b> Hãy di chuyển bàn tay nhẹ nhàng khi chụp để AI có thể học được nhiều góc độ khác nhau nhé!
              </p>
            </div>
            ...
            {submitSuccess ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-600 font-semibold mt-2">
                  Bạn đã hoàn thành tốt phần 1 bàn tay! 
                </p>
                <button
                  onClick={() => {
                    setShowSubmitModal(false);
              ...
              <div>
                <h3 className="text-xl font-black text-indigo-900 mb-2 flex items-center gap-2">
                  <span>🎒</span> Bảng Đánh Giá Bộ Dữ Liệu
                </h3>
                ...
                <div className="mb-6">
                  <label className="text-xs font-black text-gray-700 block mb-1.5">
                    Ghi chú của giáo viên (dành cho bộ dữ liệu này):
                  </label>
                  <textarea
                    rows={2}
                    value={teacherNotes}
                    onChange={(e) => setTeacherNotes(e.target.value)}
                    placeholder="Ghi chú về bộ dữ liệu của bạn..."
                ...
                <div className="flex gap-4">
                  ...
                  >
                    {isSubmitting ? (uploadProgress || 'ĐANG LƯU...') : 'XÁC NHẬN LƯU'}
                  </button>
                </div>
              </div>
            )}
