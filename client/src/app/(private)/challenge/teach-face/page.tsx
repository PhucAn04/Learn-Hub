...
const CLASSES = [
  ...
  { id: 'class_4', label: 'Bình thường (Neutral) 😐', voicePrompt: 'Hãy dạy bạn A I nhận biết nét mặt bình thường của bé nhé!' },
];
...
export default function TeachFacePage() {
  ...
  const validateExpression = (kps: { x: number; y: number }[], classId: string) => {
    ...
    if (classId === 'class_1') {
      if (detected !== 'Vui vẻ 😀') {
        return { isValid: false, suggestion: `Bé chưa cười đủ tươi! AI thấy bé đang "${detected}". Cười thật tươi lên nhé! 😀` };
      }
    } else if (classId === 'class_2') {
      if (detected !== 'Buồn bã 😢') {
        return { isValid: false, suggestion: `AI thấy bé đang "${detected}". Hãy thử làm mặt buồn nhé 😢` };
      }
    } else if (classId === 'class_3') {
      if (detected !== 'Ngạc nhiên 😲') {
        return { isValid: false, suggestion: `AI thấy bé đang "${detected}". Há miệng to và mở mắt to nhé! 😲` };
      }
    } else if (classId === 'class_4') {
      if (detected !== 'Bình thường 😐') {
        return { isValid: false, suggestion: `AI thấy bé đang "${detected}". Giữ mặt bình thường, thư giãn nhé 😐` };
      }
    }
    ...
    const timer = setTimeout(() => {
      if (predictedLabel !== speakDebounceText) {
        ...
        if (predictedLabel.includes('Vui vẻ')) {
          ...
          msg = 'A I đoán bé đang bình thường!';
          setEffectEmoji('😐');
        }
          ...
          <div className="bg-white rounded-3xl p-6 border-4 border-indigo-400 shadow-xl flex flex-col justify-between">
            <div>
              ...
              <p className="text-xs font-semibold text-gray-500 text-center leading-relaxed mb-6 bg-yellow-50 border border-yellow-200 rounded-xl p-3">
                💡 <b>Mẹo cho bé:</b> Hãy di chuyển khuôn mặt nhẹ nhàng khi chụp để AI có thể học được nhiều góc độ khác nhau nhé!
              </p>
            </div>
            ...
            <div className="bg-gradient-to-r from-indigo-900 to-purple-900 text-white rounded-3xl p-6 shadow-xl border-4 border-purple-400">
              ...
              {isTrained ? (
                ...
                <div className="text-center py-4 text-purple-200 font-bold">
                  Bé hãy chụp mẫu nét mặt bên trái rồi bấm <span className="text-yellow-300">"Dạy Bạn AI Học"</span> để xem kết quả dự đoán trực tiếp ở đây nhé! 🤖✨
                </div>
              )}
            </div>
            ...
            {submitSuccess ? (
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 font-bold mb-4">
                  Hệ thống sẽ chạy thử nghiệm chấm chéo tự động với 20 mẫu tiêu chuẩn từ Kaggle (5 mẫu mỗi cảm xúc) để đánh giá chất lượng mô hình của bé.
                </p>
                ...
                {penaltyWarning && (
                  <div className="bg-orange-50 border-2 border-orange-200 rounded-xl p-3 mb-4 text-xs font-semibold text-orange-800">
                    💡 Mẹo: Bé chụp hơi ít ảnh nên điểm bị trừ một chút (cần ít nhất 10 ảnh cho mỗi cảm xúc). Bé hãy đóng bảng này và chụp thêm nhiều góc độ khác nhau để được 100% nhé!
                  </div>
                )}
