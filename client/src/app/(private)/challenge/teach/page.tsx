...
export default function TeachAiPage() {
  ...
  const captureSample = () => {
    ...
    // Map class 3 to class 1 label, class 4 to class 2 label for KNN base classes
    ...
    // Second hand
    if (isTwoHandClass && hands[1] && hands[1].keypoints && hands[1].keypoints.length >= 21) {
      newSamples.push({
        label: knnLabel,
        features: normalizeHandKeypoints(hands[1].keypoints),
        sourceId: activeClass,
      });
    }
  ...
  const handleTrain = () => {
    ...
    if (c1 < 10 || c2 < 10 || c3 < 6 || c4 < 6) { // c3/c4 need 6 samples (3 captures x 2 hands)
      speakVietnamese('Bé chưa chụp đủ 10 ảnh mẫu cho mỗi nhóm rồi! Bé hãy chụp thêm hình mẫu nhé!');
      return;
    }
  ...
  const handleOpenSubmit = () => {
    ...
    let correctCount = 0;
    GOLDEN_TEST_DATASET.forEach(testCase => {
    ...
    let calculatedAccuracy = Math.round((correctCount / GOLDEN_TEST_DATASET.length) * 100);
    ...
    // For class_3 / class_4 each capture adds 2 samples (one per hand), so count captures = samples / 2
    sampleCounts['class_3'] = Math.floor(sampleCounts['class_3'] / 2);
    sampleCounts['class_4'] = Math.floor(sampleCounts['class_4'] / 2);

    let hasPenalty = false;
    Object.values(sampleCounts).forEach(count => {
      if (count < MIN_SAMPLES_PER_CLASS) {
        // Missing samples deduct points (e.g., 2 points per missing sample)
            ...
            <div>
              ...
              <div className="space-y-3 mb-6">
                {CLASSES.map(cls => {
                  ...
                  const classSampleCount = (cls.id === 'class_3' || cls.id === 'class_4') ? Math.floor(rawCount / 2) : rawCount;
              ...
              <button
                ...
                className="w-full bg-indigo-500 hover:bg-indigo-600 active:scale-95 disabled:bg-gray-300 disabled:scale-100 text-white font-extrabold py-4 px-6 rounded-2xl shadow-md transition-all flex items-center justify-center gap-2 text-lg mb-6 border-b-4 border-indigo-700"
              >
          ...
          <div className="bg-white rounded-3xl max-w-lg w-full p-6 border-4 border-indigo-400 shadow-2xl relative overflow-hidden">
            ...
            {submitSuccess ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-600 font-semibold mt-2">Bài nộp của bé đã được gửi tới hệ thống của thầy cô giáo.</p>
                <button
