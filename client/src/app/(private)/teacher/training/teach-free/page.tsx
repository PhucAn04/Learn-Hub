 ...
 * Giúp loại bỏ khuôn mặt, bàn tay, hoặc vật thể không thuộc nhãn đã học.
 */
const OOD_CONFIDENCE_THRESHOLD = 65; // %: dưới ngưỡng này → reject
const OOD_MAX_KNN_DISTANCE = 1.2;   // L2 distance: trên ngưỡng này → reject

export default function TeacherTeachFreePage() {
  const router = useRouter();

  // ── Custom Classes (Nhãn tự do) ──────────────────────
  ...
  // ── Hyperparameters (Tuỳ chỉnh riêng cho teach-free) ─
  ...
  const [predictionActive, setPredictionActive] = useState(false); // Trigger: true khi phát hiện đối tượng quen
  ...
  // ── Camera & MobileNet ───────────────────────────────
  const { videoRef, canvasRef, cameraError, retryCamera } = useCamera({
    width: 640,
    height: 480,
  });
  ...
  const addClass = () => {
    ...
    if (classes.some((c) => c.label === label)) {
  ...
  const captureSample = () => {
    ...
    if (!features) {
      showToast('⚠️ Không thể trích xuất đặc trưng từ ảnh. Hãy thử lại!');
      return;
    }
  ...
  // Cleanup interval khi unmount hoặc đổi class
  useEffect(() => {
    return () => {
      if (captureIntervalRef.current) {
        clearInterval(captureIntervalRef.current);
  ...
  const handleOpenSubmit = () => {
    if (!isTrained) {
      showToast('⚠️ Hãy huấn luyện mô hình trước khi lưu!');
      return;
    }
    playClickSound();
  ...
  const handleSubmit = async () => {
    try {
      ...
      setUploadProgress('');
      playSuccessSound();
      speakEnglish('Template saved successfully!');
    } catch (err) {
          ...
          <div className="bg-white rounded-3xl p-6 shadow-xl border-4 border-indigo-100 flex flex-col">
            ...
            <div className="mb-4 space-y-2">
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="Nhập tên nhãn (VD: Chó)"
                  className="flex-1 bg-slate-50 border-2 border-slate-200 rounded-xl px-3 py-2.5 text-sm font-bold text-slate-700 focus:outline-none focus:border-indigo-500 transition-colors"
                />
                <input
                  type="text"
                  value={newEmojiInput}
                  onChange={(e) => setNewEmojiInput(e.target.value)}
                  className="w-14 bg-slate-50 border-2 border-slate-200 rounded-xl px-2 py-2.5 text-center text-lg focus:outline-none focus:border-indigo-500 transition-colors"
                  placeholder="🐶"
                />
                <button
                  onClick={addClass}
                  disabled={!newLabelInput.trim() || classes.length >= MAX_CLASSES}
                  className="bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 text-white font-bold py-2.5 px-4 rounded-xl text-sm transition-colors flex items-center gap-1"
                >
              ...
              {classes.length === 0 && (
                <p className="text-xs text-slate-400 font-semibold italic">
                  💡 Hãy thêm ít nhất 2 nhãn để bắt đầu (VD: &quot;Chó 🐶&quot; và &quot;Mèo 🐱&quot;)
                </p>
              )}
            </div>
              ...
              {classes.map((c) => {
                ...
                return (
                  <button
                    ...
                    className={`relative overflow-hidden p-4 rounded-2xl border-4 transition-all text-left group ${
                      isActive
                  ...
                  >
                      ...
                      <div>
                        <div
                          className={`font-extrabold text-lg ${
                            isActive ? 'text-indigo-800' : 'text-slate-600'
                          }`}
                        >
                          {c.emoji} {c.label}
                        </div>
                      ...
                      <div className="flex items-center gap-1">
                        {count > 0 && (
                          <div
                            ...
                            className="p-2 text-slate-300 hover:text-amber-500 hover:bg-amber-50 rounded-xl transition-colors cursor-pointer"
                            title="Xóa mẫu"
                          >
                        ...
                        <div
                          onClick={(e) => {
                            e.stopPropagation();
                            if (confirm(`Xóa nhãn "${c.label}" và toàn bộ ảnh?`)) {
                              removeClass(c.id);
                            }
                          }}
                    ...
                    <div
                      className="absolute bottom-0 left-0 h-1.5 bg-indigo-500 transition-all duration-300"
            ...
            {activeClass && (
              <div className="space-y-2 mb-4">
                {/* Hold-to-Record camera capture */}
                <button
                  onPointerDown={startCapturing}
                  onPointerUp={stopCapturing}
                  onPointerLeave={stopCapturing}
                  disabled={modelStatus !== 'ready'}
                  className={`w-full font-extrabold py-3.5 px-6 rounded-2xl shadow-md transition-all flex items-center justify-center gap-2 text-lg border-b-4 active:scale-95 disabled:bg-gray-300 disabled:scale-100 select-none ${
                    isCapturing
                ...
                <button
                  ...
                  disabled={modelStatus !== 'ready'}
                  className="w-full font-extrabold py-3 px-6 rounded-2xl shadow-sm transition-all flex items-center justify-center gap-2 text-base border-b-4 bg-white hover:bg-slate-50 border-slate-200 text-indigo-700 active:scale-95 disabled:bg-gray-100 disabled:text-gray-400"
                >
            ...
            <div className="mt-auto pt-4 space-y-2">
              ...
              {isTraining ? (
                <div className="bg-indigo-50 rounded-2xl p-4 border border-indigo-100 animate-pulse">
                  ...
                  {/* Mini loss/acc display during training */}
                  {trainingLogs.length > 0 && (
                    <div className="flex gap-4 mt-2 text-[10px] font-bold text-indigo-600">
                    ...
                    <button
                      onClick={async () => {
                        try {
                          ...
                          showToast('✅ Đã tải model.json + weights.bin!');
                        } catch {
                          showToast('❌ Không thể export model.');
                        }
                      }}
          ...
          <div className="lg:col-span-2 flex flex-col gap-6">
            ...
            <div className="bg-white rounded-3xl p-6 border-4 border-indigo-400 shadow-xl relative flex flex-col items-center">
              <CameraView
                ...
                loadingText="ĐANG TẢI AI PHÂN LOẠI ẢNH..."
                theme="blue"
                onRetry={retryCamera}
              />
            ...
            <div className="bg-gradient-to-r from-indigo-900 to-purple-900 text-white rounded-3xl p-6 shadow-xl border-4 border-purple-400">
              <h4 className="font-extrabold text-[11px] text-purple-300 tracking-widest uppercase mb-3">
              ...
              {isTrained && predictionActive ? (
                <>
                  ...
                  <div className="flex justify-end mb-2">
                    <button
                      ...
                      className="text-[10px] font-bold text-purple-300 hover:text-red-300 transition-colors"
                    >
                      ✕ Tắt nhận diện
                    </button>
                  </div>
                  <div className="flex items-center justify-between gap-4">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className="text-2xl font-black drop-shadow-md text-transparent bg-clip-text bg-gradient-to-r from-green-300 to-emerald-400">
                          {predictedLabel}
                        </span>
                      </div>
                    </div>
                      ...
                      {classes.map((c) => {
                        ...
                        return (
                          <div key={c.id} className="flex items-center gap-2">
                            ...
                            <span className={`text-xs font-bold w-16 truncate ${isTop ? 'text-white' : 'text-purple-300'}`}>
                              {c.label}
                            </span>
                            <div className="flex-1 bg-white/10 rounded-full h-5 overflow-hidden relative">
                              ...
                              />
                              <span className={`absolute inset-0 flex items-center justify-end pr-2 text-[10px] font-black ${
                                pct > 50 ? 'text-white' : 'text-purple-200'
                              }`}>
                                {pct}%
                              </span>
                            </div>
                ...
                <button
                  onClick={() => setPredictionActive(true)}
                  className="w-full py-4 rounded-2xl bg-white/10 hover:bg-white/20 border border-white/20 transition-all flex items-center justify-center gap-3"
                >
                ...
                <div className="flex items-center gap-4 opacity-50 py-2">
                  ...
                  <p className="font-bold text-sm leading-relaxed">
                    Thêm nhãn, chụp ảnh, rồi huấn luyện để AI bắt đầu nhận diện! 🤖✨
                  </p>
                </div>
              )}

              {/* Self accuracy */}
              {selfAccuracy !== null && samples.length >= 6 && (
                <div className="mt-4 bg-white/5 rounded-xl p-3 border border-white/10">
                  <div className="flex items-center justify-between text-xs">
                    <span className="font-bold text-purple-200">
                      📊 Độ chính xác tự đánh giá (Leave-one-out KNN):
                    </span>
                    <span
                  ...
                  <p className="text-[10px] text-purple-300/70 italic mt-1">
                    Nếu điểm thấp, hãy chụp thêm ảnh đa dạng hoặc xóa ảnh kém chất lượng.
                  </p>
                </div>
              )}
            </div>
            ...
            {isTrained && (
              <div className="bg-white rounded-3xl p-6 shadow-xl border-4 border-amber-200">
                <h4 className="font-black text-lg text-amber-900 mb-3 flex items-center gap-2">
                  ...
                  Thử Nghiệm Bằng Ảnh Upload
                </h4>
                <p className="text-xs text-slate-500 mb-3">
                  Không có vật thật? Tải ảnh từ máy tính hoặc internet để AI dự đoán.
                </p>
                ...
                {predictImage && predictResult && (
                  <div className="mt-4 space-y-3">
                    <div className="flex gap-4 items-start">
                      {/* Image preview */}
                      <div className="w-32 h-32 rounded-2xl overflow-hidden border-2 border-slate-200 shrink-0">
                        <img src={predictImage} alt="Predict" className="w-full h-full object-cover" />
                      </div>
                      {/* Result */}
                      <div className="flex-1">
                        {predictResult.isOOD ? (
                          ...
                          <div className="bg-emerald-50 border-2 border-emerald-200 rounded-xl p-3">
                            <p className="text-xs text-slate-500 font-semibold">AI đoán đây là:</p>
                            <p className="font-black text-xl text-emerald-800 flex items-center gap-2">
                              {classes.find((c) => c.label === predictResult.label)?.emoji || '🤖'}
                              {predictResult.label}
                            </p>
                            <p className="text-sm font-bold text-emerald-600 mt-1">
                              Độ tự tin: {predictResult.confidence}%
                            </p>
                          </div>
                        )}
                      </div>
                    ...
                    {Object.keys(predictResult.confidences).length > 0 && (
                      <div className="bg-slate-50 rounded-xl p-3 space-y-1.5">
                        {classes.map((c) => {
                          ...
                          return (
                            <div key={c.id} className="flex items-center gap-2">
                              <span className="text-sm w-6 text-center">{c.emoji}</span>
                              <span className="text-[11px] font-bold w-14 truncate text-slate-600">{c.label}</span>
                              <div className="flex-1 bg-slate-200 rounded-full h-4 overflow-hidden relative">
                                <div
                                  ...
                                  style={{ width: `${Math.max(2, pct)}%` }}
                                />
                                <span className="absolute inset-0 flex items-center justify-end pr-2 text-[10px] font-black text-slate-600">
                                  {pct}%
                                </span>
                              </div>
                            </div>
            ...
            {classes.length >= 2 && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {classes.map((c) => (
                  <div
                    key={c.id}
                    className={`bg-white rounded-2xl p-4 border-2 text-center transition-all ${
                      activeClass === c.id
          ...
          <div className="bg-white rounded-3xl w-full max-w-xl overflow-hidden shadow-2xl border-4 border-white">
            <div className="bg-gradient-to-r from-indigo-500 to-purple-600 p-6 text-white relative">
              ...
              <p className="text-indigo-100 mt-2 font-medium">
                Các nhãn:{' '}
                {classes.map((c) => `${c.emoji} ${c.label}`).join(', ')}
              </p>
            </div>
              ...
              {!submitSuccess ? (
                <>
                  ...
                  <div className="bg-slate-50 rounded-xl p-4 border border-slate-200">
                    <div className="grid grid-cols-2 gap-3 text-sm">
                      <div>
                        <span className="text-slate-500 font-semibold">Số nhãn:</span>
                      ...
                      {selfAccuracy !== null && (
                        <div className="col-span-2">
                          <span className="text-slate-500 font-semibold">Độ chính xác:</span>
                          <span
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder="VD: Hãy chụp ảnh đa dạng góc chụp nhé!"
                    />
                  </div>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-slate-500 font-medium mt-2">
                    Học sinh có thể tìm thấy bài tập này trong phần Sandbox.
                  </p>
                  <button
